...
-(void)configBanner:(NSArray *)banners{
    ...
    _imageArray=[[NSMutableArray alloc]initWithCapacity:banners.count];
    for (SlideModel *slidemodel in banners) {
       
        [_imageArray addObject:slidemodel.slide_picture];
    }


    self.pageController.numberOfPages = banners.count;
...
- (void)initSubviews{
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-80-[_pageController]" options:0 metrics:0 views:NSDictionaryOfVariableBindings(_pageController)]];
    [self addTimer];
...
- (UIPageControl *)pageController{
    if (!_pageController) {
        ...
        _pageController.currentPageIndicatorTintColor = [UIColor navBgColor];
