...
class CloudsearchClient  {
   ...
   */
  public function __construct( $opts = array(),$key_type = 'aliyun') {

    $this->key_type = $key_type;

    if ($this->key_type == 'opensearch'){
       $this->clientId = C('SEARCH_KEY');
       $this->clientSecret =  C('SEARCH_SECRET');
       
       
    } elseif ($this->key_type == 'aliyun'){
       $this->accessKeyId = C('SEARCH_KEY');
       $this->secret = C('SEARCH_SECRET');
    } else {
  ...
  private function _socket($url, $params = array(), $method = self::METHOD) {
    ...
    if (!$socket) {
      throw new   \Think\Exception("Connect " . $parse['host'] . ' fail.');
    }
  ...
  private function _parseUrl($url) {
    ...
    if (empty($parse) || !is_array($parse)) {
      throw new \Think\Exception("Host is empty.");
    }
