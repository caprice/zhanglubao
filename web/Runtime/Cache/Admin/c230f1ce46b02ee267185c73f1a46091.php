...
<h2><?php echo isset($series['id'])?'编辑':'新增';?>系列赛</h2>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="title" value="<?php echo ((isset($series["title"]) && ($series["title"] !== ""))?($series["title"]):''); ?>"></div>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="live_id" value="<?php echo ((isset($series["live_id"]) && ($series["live_id"] !== ""))?($series["live_id"]):''); ?>"></div>
</div>
...
<div class="controls"><select name="game_id">
	<?php if(is_array($games)): $i = 0; $__LIST__ = $games;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$game): $mod = ($i % 2 );++$i;?><option value="<?php echo ($game["id"]); ?>"<?php if(($series['game_id']) == $game["id"]): ?>selected<?php endif; ?>><?php echo ($game["title_show"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select></div>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="team_id" value="<?php echo ((isset($series["team_id"]) && ($series["team_id"] !== ""))?($series["team_id"]):''); ?>"></div>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="tags" value="<?php echo ((isset($series["tags"]) && ($series["tags"] !== ""))?($series["tags"]):''); ?>"></div>
</div>

<div class="form-item"><label class="item-label">系列赛封面</label> <input
	type="file" id="upload_picture"> <input type="hidden"
	name="cover" id="cover" value="<?php echo ((isset($series['cover']) && ($series['cover'] !== ""))?($series['cover']):''); ?>" />
<div class="upload-img-box"><?php if(!empty($series['cover'])): ?><div class="upload-pre-item"><img
	src="<?php echo (get_cover($series["cover"],'url')); ?>" /></div><?php endif; ?></div>
</div>
<script type="text/javascript">
					//上传图片
				    /* 初始化上传插件 */
					$("#upload_picture").uploadify({
				        ...
				        "buttonText"      : "上传图片",
				        "uploader"        : "<?php echo U('File/uploadSeriesPic',array('session_id'=>session_id()));?>",
				        "width"           : 120,
				        'removeTimeout'	  : 1,
...
<textarea name="description"><?php echo ((isset($series["description"]) && ($series["description"] !== ""))?($series["description"]):''); ?></textarea>
</label></div>
</div>
<div class="form-item"><input type="hidden" name="id"
	value="<?php echo ((isset($series["id"]) && ($series["id"] !== ""))?($series["id"]):''); ?>">
