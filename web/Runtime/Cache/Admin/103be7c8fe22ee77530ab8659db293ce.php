...
<h2><?php echo isset($master['id'])?'编辑':'新增';?>大师</h2>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="uid" value="<?php echo ((isset($master["uid"]) && ($master["uid"] !== ""))?($master["uid"]):''); ?>"></div>
</div>
...
<div class="controls"><select name="game_id">
	<?php if(is_array($games)): $i = 0; $__LIST__ = $games;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$game): $mod = ($i % 2 );++$i;?><option value="<?php echo ($game["id"]); ?>"<?php if(($master['game_id']) == $game["id"]): ?>selected<?php endif; ?>><?php echo ($game["title_show"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select></div>
</div>
<div class="form-item"><label class="item-label">大师标签<span
	class="check-tips">（大师标签）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="tags" value="<?php echo ((isset($master["tags"]) && ($master["tags"] !== ""))?($master["tags"]):''); ?>"></div>
</div>

<div class="form-item"><label class="item-label">游戏ID<span
	class="check-tips">（游戏ID）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="game_nick_name" value="<?php echo ((isset($master["game_nick_name"]) && ($master["game_nick_name"] !== ""))?($master["game_nick_name"]):''); ?>">
</div>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="honor_title" value="<?php echo ((isset($master["honor_title"]) && ($master["honor_title"] !== ""))?($master["honor_title"]):''); ?>"></div>
</div>

<div class="form-item"><label class="item-label">游戏位置<span
	class="check-tips">（游戏位置）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="game_place" value="<?php echo ((isset($master["game_place"]) && ($master["game_place"] !== ""))?($master["game_place"]):''); ?>"></div>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="team" value="<?php echo ((isset($master["team"]) && ($master["team"] !== ""))?($master["team"]):''); ?>"></div>
</div>

<div class="form-item"><label class="item-label">微博<span
	class="check-tips">（微博）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="weibo" value="<?php echo ((isset($master["weibo"]) && ($master["weibo"] !== ""))?($master["weibo"]):''); ?>"></div>
</div>
...
<div class="form-item"><label class="item-label">大师图腾</label> <input
	...
	name="cover" id="cover" value="<?php echo ((isset($master['cover']) && ($master['cover'] !== ""))?($master['cover']):''); ?>" />
<div class="upload-img-box"><?php if(!empty($master['cover'])): ?><div class="upload-pre-item"><img
	src="<?php echo (get_cover($master["cover"],'url')); ?>" /></div><?php endif; ?></div>
</div>
<script type="text/javascript">
					//上传图片
				    /* 初始化上传插件 */
					$("#upload_picture").uploadify({
				        ...
				        "buttonText"      : "上传图片",
				        "uploader"        : "<?php echo U('File/uploadMasterPic',array('session_id'=>session_id()));?>",
				        "width"           : 120,
				        'removeTimeout'	  : 1,
...
<textarea name="description"><?php echo ((isset($master["description"]) && ($master["description"] !== ""))?($master["description"]):''); ?></textarea>
</label></div>
</div>
<div class="form-item"><input type="hidden" name="id"
	value="<?php echo ((isset($master["id"]) && ($master["id"] !== ""))?($master["id"]):''); ?>">
...
<script type="text/javascript">
        //导航高亮
        highlight_subnav('<?php echo U('Commentator/index');?>');
    </script>
