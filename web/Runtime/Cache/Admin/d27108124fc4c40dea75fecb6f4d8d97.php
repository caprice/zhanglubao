...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="main-title">
        <h2><?php echo isset($team['id'])?'编辑':'新增';?>战队</h2>
    </div>
    ...
    <form action="<?php echo U();?>" method="post" class="form-horizontal" >
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="name" value="<?php echo ((isset($team["name"]) && ($team["name"] !== ""))?($team["name"]):''); ?>">
            </div>
        </div>
        ...
        <div class="form-item">
            <label class="item-label">归属游戏<span class="check-tips">（是什么游戏的战队）</span></label>
                    <div class="controls">
                <select name="game_id">
                    <?php if(is_array($games)): $i = 0; $__LIST__ = $games;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$game): $mod = ($i % 2 );++$i;?><option value="<?php echo ($game["id"]); ?>"  <?php if(($team['game_id']) == $game["id"]): ?>selected<?php endif; ?>><?php echo ($game["title_show"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
             <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="tags" value="<?php echo ((isset($team["tags"]) && ($team["tags"] !== ""))?($team["tags"]):''); ?>">
            </div>
        </div>
         ...
         <div class="form-item">
						...
						<input type="hidden" name="cover" id="cover" value="<?php echo ((isset($team['cover']) && ($team['cover'] !== ""))?($team['cover']):''); ?>"/>
						<div class="upload-img-box">
						<?php if(!empty($team['cover'])): ?><div class="upload-pre-item"><img src="<?php echo (get_cover($team["cover"],'url')); ?>"/></div><?php endif; ?>
						</div>
					</div>
					...
					$("#upload_picture").uploadify({
				        ...
				        "buttonText"      : "上传图片",
				        "uploader"        : "<?php echo U('File/uploadTeamPic',array('session_id'=>session_id()));?>",
				        "width"           : 120,
				        'removeTimeout'	  : 1,
           ...
           <div class="form-item">
            ...
            <label class="textarea input-large">
				<textarea name="description"><?php echo ((isset($team["description"]) && ($team["description"] !== ""))?($team["description"]):''); ?></textarea>
			</label>
            </div>
        </div>
            ...
            <div class="controls">
            <label class="textarea input-large">
				<textarea name="honor"><?php echo ((isset($team["honor"]) && ($team["honor"] !== ""))?($team["honor"]):''); ?></textarea>
			</label>
            </div>
        </div>
        <div class="form-item">
        <input type="hidden" name="id" value="<?php echo ((isset($team["id"]) && ($team["id"] !== ""))?($team["id"]):''); ?>">
