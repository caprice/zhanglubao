...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="cf">
        <div class="fl">
            <a class="btn" href="<?php echo U('Live/add');?>">新 增</a>
            <button class="btn ajax-post confirm" url="<?php echo U('Live/changeStatus',array('method'=>'deleteLive'));?>"
                    target-form="ids">删 除
            </button>
        </div>
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="nickname" class="search-input" value="<?php echo I('title');?>"
        ...
        <table class="">
            ...
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                     ...
                     <td><?php echo get_game($vo['game_id'],'title');?></td>
                        ...
                        <td>
                            <?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Live/changeStatus?method=forbidLive&id='.$vo['id']);?>"
                                   class="ajax-get">禁用</a>
                                <?php else: ?>
                                <a href="<?php echo U('Live/changeStatus?method=resumeLive&id='.$vo['id']);?>"
                                   class="ajax-get">启用</a><?php endif; ?>
                            <a href="<?php echo U('Live/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
                            <a href="<?php echo U('Live/changeStatus?method=deleteLive&id='.$vo['id']);?>"
