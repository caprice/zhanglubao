...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="cf">
        <div class="fl">
            <button class="btn ajax-post confirm" url="<?php echo U('Match/changeStatus',array('method'=>'deleteMatch'));?>"
                    target-form="ids">删 除
            </button>
        </div>
        ...
        <table class="">
            ...
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                       ...
                       <td><?php echo date('Y-m-d H:i:s',$vo['update_time']);?></td>
                        ...
                        <td>
                            <?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Match/changeStatus?method=forbidMatch&id='.$vo['id']);?>"
                                   class="ajax-get">禁用</a>
                                <?php else: ?>
                                <a href="<?php echo U('Match/changeStatus?method=resumeMatch&id='.$vo['id']);?>"
                                   class="ajax-get">启用</a><?php endif; ?>
                            <a href="<?php echo U('Match/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
                            <a href="<?php echo U('Match/changeStatus?method=deleteMatch&id='.$vo['id']);?>"
