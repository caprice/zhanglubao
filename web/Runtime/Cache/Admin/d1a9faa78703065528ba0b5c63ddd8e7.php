...
<div class="fl"><a class="btn"
	href="<?php echo U('Ad/add');?>">新 增</a>
<button class="btn ajax-post confirm"
	url="<?php echo U('Ad/changeStatus',array('method'=>'deleteAd'));?>"
	...
	<tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo ($vo["title"]); ?></td>
			 <td>
			 <?php switch($vo['place']): case "0": ?>滑动广告<?php break;?> <?php case "1": ?>小框广告<?php break;?> 
		
		<?php case "2": ?>其他广告<?php break; endswitch;?>
			...
			<?php if(($vo["status"]) == "1"): ?><a
				href="<?php echo U('Ad/changeStatus?method=forbidAd&id='.$vo['id']);?>"
				class="ajax-get">禁用</a> <?php else: ?> <a
				href="<?php echo U('Ad/changeStatus?method=resumeAd&id='.$vo['id']);?>"
				class="ajax-get">启用</a><?php endif; ?> <a
				href="<?php echo U('Ad/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
			<a
				href="<?php echo U('Ad/changeStatus?method=deleteAd&id='.$vo['id']);?>"
