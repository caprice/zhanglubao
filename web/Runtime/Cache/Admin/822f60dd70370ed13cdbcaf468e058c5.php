...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="cf">
        <div class="fl">
            <a class="btn" href="<?php echo U('Commentator/add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Commentator/changeStatus',array('method'=>'resumeCommentator'));?>"
                    target-form="ids">启 用
            </button>
            <button class="btn ajax-post" url="<?php echo U('Commentator/changeStatus',array('method'=>'forbidCommentator'));?>"
                    target-form="ids">禁 用
            </button>
            <button class="btn ajax-post confirm" url="<?php echo U('Commentator/changeStatus',array('method'=>'deleteCommentator'));?>"
                    target-form="ids">删 除
            </button>
        </div>
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="nickname" class="search-input" value="<?php echo I('nickname');?>"
    ...
    <div class="data-table table-striped">
        <table class="">
            ...
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Commentator/changeStatus?method=forbidCommentator&id='.$vo['uid']);?>"
                                   class="ajax-get">禁用</a>
                                <?php else: ?>
                                <a href="<?php echo U('Commentator/changeStatus?method=resumeCommentator&id='.$vo['uid']);?>"
                                   class="ajax-get">启用</a><?php endif; ?>
                                   <a href="<?php echo U('Commentator/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
                            <a href="<?php echo U('Commentator/changeStatus?method=deleteCommentator&id='.$vo['uid']);?>"
