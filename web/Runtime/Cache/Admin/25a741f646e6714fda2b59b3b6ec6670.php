...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="main-title">
        <h2><?php echo isset($live['id'])?'编辑':'新增';?>房间</h2>
    </div>
    ...
    <form action="<?php echo U();?>" method="post" class="form-horizontal" >
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="title" value="<?php echo ((isset($live["title"]) && ($live["title"] !== ""))?($live["title"]):''); ?>">
            </div>
        </div>
              <div class="form-item">
            <label class="item-label">房间标签<span class="check-tips">（房间标签）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="tags" value="<?php echo ((isset($live["tags"]) && ($live["tags"] !== ""))?($live["tags"]):''); ?>">
            </div>
        </div>
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="uid" value="<?php echo ((isset($live["uid"]) && ($live["uid"] !== ""))?($live["uid"]):'1'); ?>">
            </div>
        </div>
       ...
       <div class="form-item">
            <label class="item-label">来之直播<span class="check-tips">（是什么游戏的房间）</span></label>
                ...
                <select name="from_site">
                    	  <?php if(is_array($sites)): $i = 0; $__LIST__ = $sites;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$site): $mod = ($i % 2 );++$i;?><option value="<?php echo ($site['id']); ?>"  <?php if(($$site['from_site']) == $live["from_site"]): ?>selected<?php endif; ?>><?php echo ($site["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="flash_url" value="<?php echo ((isset($live["flash_url"]) && ($live["flash_url"] !== ""))?($live["flash_url"]):''); ?>">
            </div>
        </div>
        ...
        <div class="form-item">
            <label class="item-label">归属游戏<span class="check-tips">（是什么游戏的房间）</span></label>
                    <div class="controls">
                <select name="game_id">
                    <?php if(is_array($games)): $i = 0; $__LIST__ = $games;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$game): $mod = ($i % 2 );++$i;?><option value="<?php echo ($game["id"]); ?>"  <?php if(($live['game_id']) == $game["id"]): ?>selected<?php endif; ?>><?php echo ($game["title_show"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
         ...
         <div class="form-item">
						...
						<input type="hidden" name="cover" id="cover" value="<?php echo ((isset($live['cover']) && ($live['cover'] !== ""))?($live['cover']):''); ?>"/>
						<div class="upload-img-box">
						<?php if(!empty($live['cover'])): ?><div class="upload-pre-item"><img src="<?php echo (get_cover($live["cover"],'url')); ?>"/></div><?php endif; ?>
						</div>
					</div>
					...
					$("#upload_picture").uploadify({
				        ...
				        "buttonText"      : "上传图片",
				        "uploader"        : "<?php echo U('File/uploadLivePic',array('session_id'=>session_id()));?>",
				        "width"           : 120,
				        'removeTimeout'	  : 1,
           ...
           <div class="form-item">
            ...
            <label class="textarea input-large">
				<textarea name="description"><?php echo ((isset($live["description"]) && ($live["description"] !== ""))?($live["description"]):''); ?></textarea>
			</label>
            </div>
        </div>
        <div class="form-item">
        <input type="hidden" name="id" value="<?php echo ((isset($live["id"]) && ($live["id"] !== ""))?($live["id"]):''); ?>">
