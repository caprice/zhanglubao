...
<h2><?php echo isset($category['id'])?'编辑':'新增';?>分类</h2>
</div>
<form action="<?php echo U();?>" method="post" class="form-horizontal">
<div class="form-item"><label class="item-label">分类名称<span
	class="check-tips">（分类名称昵称）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="name" value="<?php echo ((isset($category["name"]) && ($category["name"] !== ""))?($category["name"]):''); ?>"></div>
</div>

<div class="form-item"><label class="item-label">玩法<span
	class="check-tips">（玩法）</span></label>
<div class="controls"> 
<ul>
	<?php if(is_array($playways)): $i = 0; $__LIST__ = $playways;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$playway): $mod = ($i % 2 );++$i;?><li><input type="checkbox" name="play_ways[]" value="<?php echo ($playway["id"]); ?>" <?php if(in_array($playway['id'],$category['play_ways'])): ?>checked<?php endif; ?>><?php echo ($playway["name"]); ?>
...
<div class="form-item"><label class="item-label">竞猜参数个数<span
	class="check-tips">（竞猜参数个数）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="parameter_count" value="<?php echo ((isset($category["parameter_count"]) && ($category["parameter_count"] !== ""))?($category["parameter_count"]):''); ?>">
</div>
</div>

<div class="form-item"><label class="item-label">启用状态<span
	class="check-tips">（启用状态）</span></label>
<div class="controls"><select name="status">
	<option value="0"<?php if(($category['status']) == "0"): ?>selected<?php endif; ?>>不启用</option>
	<option value="1"<?php if(($category['status']) == "1"): ?>selected<?php endif; ?>>启用</option>
</select></div>
</div>


<div class="form-item"><label class="item-label">固定赔率<span
	class="check-tips">（固定赔率）</span></label>
<div class="controls"><select name="status">
	<option value="0"<?php if(($category['status']) == "0"): ?>selected<?php endif; ?>>不启用</option>
	<option value="1"<?php if(($category['status']) == "1"): ?>selected<?php endif; ?>>启用</option>
</select></div>
...
<div class="form-item"><label class="item-label">浮动赔率<span
	class="check-tips">（浮动赔率）</span></label>
<div class="controls"><select name="status">
	<option value="0"<?php if(($category['status']) == "0"): ?>selected<?php endif; ?>>不启用</option>
	<option value="1"<?php if(($category['status']) == "1"): ?>selected<?php endif; ?>>启用</option>
</select></div>
...
<div class="form-item"><label class="item-label">排序<span
	class="check-tips">（排序）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="sort_num" value="<?php echo ((isset($category["sort_num"]) && ($category["sort_num"] !== ""))?($category["sort_num"]):'0'); ?>"></div>
</div>


<div class="form-item"><input type="hidden" name="id"
	value="<?php echo ((isset($category["id"]) && ($category["id"] !== ""))?($category["id"]):''); ?>">
