...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="main-title">
        <h2><?php echo isset($liverecommend['id'])?'编辑':'新增';?>推荐</h2>
    </div>
    ...
    <form action="<?php echo U();?>" method="post" class="form-horizontal" >
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="title" value="<?php echo ((isset($liverecommend["title"]) && ($liverecommend["title"] !== ""))?($liverecommend["title"]):''); ?>">
            </div>
        </div>
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="live_id" value="<?php echo ((isset($liverecommend["live_id"]) && ($liverecommend["live_id"] !== ""))?($liverecommend["live_id"]):'1'); ?>">
            </div>
        </div>
         ...
         <div class="form-item">
						...
						<input type="hidden" name="cover" id="cover" value="<?php echo ((isset($liverecommend['cover']) && ($liverecommend['cover'] !== ""))?($liverecommend['cover']):''); ?>"/>
						<div class="upload-img-box">
						<?php if(!empty($liverecommend['cover'])): ?><div class="upload-pre-item"><img src="<?php echo (get_cover($liverecommend["cover"],'url')); ?>"/></div><?php endif; ?>
						</div>
					</div>
					...
					$("#upload_picture").uploadify({
				        ...
				        "buttonText"      : "上传图片",
				        "uploader"        : "<?php echo U('File/uploadLiveRecommendPic',array('session_id'=>session_id()));?>",
				        "width"           : 120,
				        'removeTimeout'	  : 1,
        ...
        <div class="form-item">
        <input type="hidden" name="id" value="<?php echo ((isset($liverecommend["id"]) && ($liverecommend["id"] !== ""))?($liverecommend["id"]):''); ?>">
