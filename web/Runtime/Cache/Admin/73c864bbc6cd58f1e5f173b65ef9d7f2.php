...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="cf">
        <div class="fl">
            <a class="btn" href="<?php echo U('LiveRecommend/add');?>">新 增</a>
            <button class="btn ajax-post confirm" url="<?php echo U('LiveRecommend/changeStatus',array('method'=>'deleteLiveRecommend'));?>"
                    target-form="ids">删 除
            </button>
        </div>
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>"
                       placeholder="标题">
    ...
    <div class="data-table table-striped">
        <table class="">
            ...
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('LiveRecommend/changeStatus?method=forbidLiveRecommend&id='.$vo['id']);?>"
                                   class="ajax-get">禁用</a>
                                <?php else: ?>
                                <a href="<?php echo U('LiveRecommend/changeStatus?method=resumeLiveRecommend&id='.$vo['id']);?>"
                                   class="ajax-get">启用</a><?php endif; ?>
                            <a href="<?php echo U('LiveRecommend/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
                            <a href="<?php echo U('LiveRecommend/changeStatus?method=deleteLiveRecommend&id='.$vo['id']);?>"
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
        ...
        //导航高亮
        highlight_subnav("<?php echo U('Live / index');?>");
    </script>
