...
<h2><?php echo isset($commentator['id'])?'编辑':'新增';?>解说</h2>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="uid" value="<?php echo ((isset($commentator["uid"]) && ($commentator["uid"] !== ""))?($commentator["uid"]):''); ?>"></div>
</div>
...
<div class="controls"><select name="game_id">
	<?php if(is_array($games)): $i = 0; $__LIST__ = $games;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$game): $mod = ($i % 2 );++$i;?><option value="<?php echo ($game["id"]); ?>"<?php if(($commentator['game_id']) == $game["id"]): ?>selected<?php endif; ?>><?php echo ($game["title_show"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select></div>
</div>
<div class="form-item"><label class="item-label">解说标签<span
	class="check-tips">（解说名称昵称）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="tags" value="<?php echo ((isset($commentator["tags"]) && ($commentator["tags"] !== ""))?($commentator["tags"]):''); ?>"></div>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="name" value="<?php echo ((isset($commentator["name"]) && ($commentator["name"] !== ""))?($commentator["name"]):''); ?>"></div>
</div>
...
<div class="controls"><input type="text" class="text input-large"
	name="weibo" value="<?php echo ((isset($commentator["weibo"]) && ($commentator["weibo"] !== ""))?($commentator["weibo"]):''); ?>"></div>
</div>

<div class="form-item"><label class="item-label">解说游戏昵称<span
	class="check-tips">（解说游戏昵称）</span></label>
<div class="controls"><input type="text" class="text input-large"
	name="game_nick_name" value="<?php echo ((isset($commentator["game_nick_name"]) && ($commentator["game_nick_name"] !== ""))?($commentator["game_nick_name"]):''); ?>"></div>
</div>
...
<textarea name="c_style"><?php echo ((isset($commentator["c_style"]) && ($commentator["c_style"] !== ""))?($commentator["c_style"]):''); ?></textarea>
</label>
</div>
</div>
...
<textarea name="description"><?php echo ((isset($commentator["description"]) && ($commentator["description"] !== ""))?($commentator["description"]):''); ?></textarea>
</label></div>
</div>

<div class="form-item"><label class="item-label">解说图腾</label> <input
	type="file" id="upload_picture"> <input type="hidden"
	name="cover" id="cover" value="<?php echo ((isset($commentator['cover']) && ($commentator['cover'] !== ""))?($commentator['cover']):''); ?>" />
<div class="upload-img-box"><?php if(!empty($commentator['cover'])): ?><div class="upload-pre-item"><img
	src="<?php echo (get_cover($commentator["cover"],'url')); ?>" /></div><?php endif; ?></div>
</div>
<script type="text/javascript">
					//上传图片
				    /* 初始化上传插件 */
					$("#upload_picture").uploadify({
				        ...
				        "buttonText"      : "上传图片",
				        "uploader"        : "<?php echo U('File/uploadCommentatorPic',array('session_id'=>session_id()));?>",
				        "width"           : 120,
				        'removeTimeout'	  : 1,
...
<div class="form-item"><input type="hidden" name="id"
	value="<?php echo ((isset($commentator["id"]) && ($commentator["id"] !== ""))?($commentator["id"]):''); ?>">
...
<script type="text/javascript">
        //导航高亮
        highlight_subnav('<?php echo U('Commen/index');?>');
    </script>
