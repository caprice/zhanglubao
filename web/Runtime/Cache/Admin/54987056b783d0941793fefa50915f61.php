...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="cf">
        <div class="fl">
            <a class="btn" href="<?php echo U('Team/add');?>">新 增</a>
            <button class="btn ajax-post confirm" url="<?php echo U('Team/changeStatus',array('method'=>'deleteTeam'));?>"
                    target-form="ids">删 除
            </button>
        </div>
    ...
    <div class="data-table table-striped">
        <table class="">
            ...
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo ($vo["members"]); ?></td>
                        <td><?php echo ($vo["status"]); ?></td>
                        <td>
                         <a href="<?php echo U('Team/addMember?id='.$vo['id']);?>" class="authorize">添加成员</a>
                            <?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Team/changeStatus?method=forbidTeam&id='.$vo['id']);?>"
                                   class="ajax-get">禁用</a>
                                <?php else: ?>
                                <a href="<?php echo U('Team/changeStatus?method=resumeTeam&id='.$vo['id']);?>"
                                   class="ajax-get">启用</a><?php endif; ?>
                            <a href="<?php echo U('Team/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
                            <a href="<?php echo U('Team/changeStatus?method=deleteTeam&id='.$vo['id']);?>"
