<?php if (!defined('THINK_PATH')) exit(); if(is_array($tree)): $i = 0; $__LIST__ = $tree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><dl class="cate-item">
		<dt class="cf">
			<form action="<?php echo U('add');?>" method="post">
				<div class="btn-toolbar opt-btn cf">
					<a title="编辑" href="<?php echo U('add?id='.$list['id'].'&pid='.$list['pid']);?>">编辑</a>
					...
					<a title="删除" href="<?php echo U('setStatus?ids='.$list['id'].'&status=-1');?>" class="confirm ajax-get">删除</a>
		...
		<?php if(!empty($list['_'])): ?><dd>
                <?php $tree_list = new Admin\Builder\AdminTreeListBuilder(); $tree_list->tree($list['_']); ?>
			</dd><?php endif; ?>
