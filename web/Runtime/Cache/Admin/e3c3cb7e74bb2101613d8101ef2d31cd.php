...
<div class="fl"><a class="btn"
	href="<?php echo U('Fight/add');?>">新 增</a>
<button class="btn ajax-post confirm"
	url="<?php echo U('Fight/changeStatus',array('method'=>'deleteFight'));?>"
...
<table class="">
	...
	<tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo get_username($vo['host_uid']);?></td>
			<td><?php echo get_username($vo['guest_uid']);?></td>
			...
			<?php if(($vo["status"]) == "1"): ?><a
				href="<?php echo U('Fight/changeStatus?method=forbidFight&id='.$vo['id']);?>"
				class="ajax-get">禁用</a> <?php else: ?> <a
				href="<?php echo U('Fight/changeStatus?method=resumeFight&id='.$vo['id']);?>"
				class="ajax-get">启用</a><?php endif; ?> <a
				href="<?php echo U('Fight/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
			<a
				href="<?php echo U('Fight/changeStatus?method=deleteFight&id='.$vo['id']);?>"
