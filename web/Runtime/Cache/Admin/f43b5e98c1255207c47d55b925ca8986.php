...
<?php if(!empty($_show_nav)): ?><div class="breadcrumb"><span>您的位置:</span> <?php $i = '1'; ?> <?php if(is_array($_nav)): foreach($_nav as $k=>$v): if($i == count($_nav)): ?><span><?php echo ($v); ?></span> <?php else: ?> <span><a
    ...
    <div class="cf">
        <div class="fl">
            <a class="btn" href="<?php echo U('Master/add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Master/changeStatus',array('method'=>'resumeMaster'));?>"
                    target-form="ids">启 用
            </button>
            <button class="btn ajax-post" url="<?php echo U('Master/changeStatus',array('method'=>'forbidMaster'));?>"
                    target-form="ids">禁 用
            </button>
            <button class="btn ajax-post confirm" url="<?php echo U('Master/changeStatus',array('method'=>'deleteMaster'));?>"
                    target-form="ids">删 除
            </button>
        </div>
    ...
    <div class="data-table table-striped">
        <table class="">
            ...
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Master/changeStatus?method=forbidMaster&id='.$vo['uid']);?>"
                                   class="ajax-get">禁用</a>
                                <?php else: ?>
                                <a href="<?php echo U('Master/changeStatus?method=resumeMaster&id='.$vo['uid']);?>"
                                   class="ajax-get">启用</a><?php endif; ?>
                                   <a href="<?php echo U('Master/edit?id='.$vo['id']);?>" class="authorize">编辑</a>
                            <a href="<?php echo U('Master/changeStatus?method=deleteMaster&id='.$vo['uid']);?>"
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
        ...
        //导航高亮
        highlight_subnav("<?php echo U('Master / index');?>");
    </script>
