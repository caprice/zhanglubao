...
<div class="cf">
<div class="fl"><a class="btn" href="<?php echo U('Video/add');?>">新 增</a>
<button class="btn ajax-post confirm"
	url="<?php echo U('Video/changeStatus',array('method'=>'deleteVideo'));?>"
...
<div class="sleft"><input type="text" name="nickname"
	class="search-input" value="<?php echo I('nickname');?>" placeholder="视频标题">
...
<table class="">
	...
	<tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo (op_t($vo["title"])); ?></td>
			<td><?php echo get_game($vo['game_id'],'title');?></td>
			<td><?php echo date('Y-m-d H:i:s',$vo['update_time']);?></td>
			<td><?php echo ($vo["status"]); ?></td>
			<td><?php echo get_username($vo['uid']);?></td>
			<td>
			
			<?php if(($vo["edit_status"]) == "1"): ?><a
				href="<?php echo U('Video/changeEditStatus?method=unrecVideo&id='.$vo['id']);?>"
				class="ajax-get">不推荐</a> <?php else: ?> <a
				href="<?php echo U('Video/changeEditStatus?method=recVideo&id='.$vo['id']);?>"
				class="ajax-get">推荐</a><?php endif; ?>
				
				
			
			<?php if(($vo["status"]) == "1"): ?><a
				href="<?php echo U('Video/changeStatus?method=forbidVideo&id='.$vo['id']);?>"
				class="ajax-get">禁用</a> <?php else: ?> <a
				href="<?php echo U('Video/changeStatus?method=resumeVideo&id='.$vo['id']);?>"
				class="ajax-get">启用</a><?php endif; ?> <a href="<?php echo U('Video/edit?id='.$vo['id']);?>"
				class="authorize">编辑</a> <a
				href="<?php echo U('Video/changeStatus?method=deleteVideo&id='.$vo['id']);?>"
