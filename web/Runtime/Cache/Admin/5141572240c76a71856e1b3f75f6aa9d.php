...
<h2><?php echo isset($match['id'])?'编辑':'新增';?>视频</h2>
</div>
...
<div class="form-item"><input type="hidden" name="id"
	value="<?php echo ((isset($match["id"]) && ($match["id"] !== ""))?($match["id"]):''); ?>"> <?php if($match): ?><input type="hidden" name="match_id"
	value="<?php echo ((isset($match["match_id"]) && ($match["match_id"] !== ""))?($match["match_id"]):''); ?>"> <?php else: ?> <input
