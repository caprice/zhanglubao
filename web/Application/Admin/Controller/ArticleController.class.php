...
class ArticleController extends AdminController {
    ...
    protected function checkDynamic(){
        ...
        switch(strtolower(ACTION_NAME)){
            ...
            case 'update':  //更新
                $doc_id  =  I('id');
                $cate_id =  M('Document')->where(array('id'=>$doc_id))->getField('game_id');
                break;
            case 'setstatus': //更改状态
            case 'permit':    //回收站
                $doc_id  =  (array)I('ids');
                $cate_id =  M('Document')->where(array('id'=>array('in',$doc_id)))->getField('game_id',true);
    ...
    protected function getMenu(){
        ...
        $cate       =   M('Game')->where(array('status'=>1))->field('id,title,pid,allow_publish')->order('pid,sort')->select();
    ...
    public function index($cate_id = null){
        ...
        if(!empty($cate_id)){   //没有权限则不查询数据
            //获取分类绑定的模型
            $models         =   get_game($cate_id, 'model');
            $allow_reply    =   get_game($cate_id, 'reply');//分类文档允许回复
            $pid            =   I('pid');
            if ( $pid==0 ) {
    ...
    protected function indexOfReply($cate_id) {
        ...
        $_REQUEST = array();
        $list = $this->lists($list,null,null,null,'l.id id,l.pid pid,l.game_id,l.title title,l.update_time update_time,l.uid uid,l.status status,r.content content' );
        int_to_string($list);
    ...
    public function edit(){
        ...
        //获取当前分类的文档类型
        $this->assign('type_list', get_type_bycate($data['game_id']));
    ...
    public function examine(){
        ...
        if ( !IS_ROOT ) {
            ...
            if($cate_auth){
                $map['game_id']    =   array('IN',$cate_auth);
            }else{
                $map['game_id']    =   -1;
            }
        }
    ...
    protected function checkPaste($list, $cate_id, $pid){
        ...
        // 检查支持的文档模型
        $modelList =   M('Game')->getFieldById($cate_id,'model');	// 当前分类支持的文档模型
        ...
        // 检查支持的文档类型和层级规则
        $typeList =   M('Game')->getFieldById($cate_id,'type');	// 当前分类支持的文档模型
        foreach ($list as $key=>$value){
            // 移动文档的所属文档模型
