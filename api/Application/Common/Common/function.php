...
function  get_game_title($game_id)
{
	...
	if (isset($list[$key])) {
		...
		$Game = M('SystemGame');
		$info = $Game->where('id='.$game_id)->find();

		if ($info && isset($info['title'])) {
			$game_title = $list[$key] = $info['title'];
			S('sys_game_title_list', $list);
		} else {
			$game_title = '';
		}
	}
...
function  get_game_name($game_id)
{
	...
	if (isset($list[$key])) {
		...
		if ($info && isset($info['name'])) {
			$game_name = $list[$key] = $info['name'];
			S('sys_game_name_list', $list);
		} else {
			$game_name = '';
		}
	}
...
function  get_country_name($country_id)
{
	static $list;
	if (empty($list)) {
		$list = S('sys_country_list');
	}
	 
	$key = "c{$country_id}";
	if (isset($list[$key])) {  
		$country_name = $list[$key];
	
	} else {
		$Country = M('SystemCountry');
		$info = $Country->where('id='.$country_id)->find();
	
		if ($info && isset($info['country_name'])) {
			$country_name = $list[$key] = $info['country_name'];
			S('sys_country_list', $list);
		} else {
			$country_name = '';
		}
	}
...
function get_group_name($group_id)
{
	...
	if (isset($list[$key])) {  
		...
		$Group = M('UserGroup');
		$info = $Group->where('id='.$group_id)->find();
	
		if ($info && isset($info['group_name'])) {
			$group_name = $list[$key] = $info['group_name'];
			S('sys_group_list', $list);
		} else {
			$group_name = '';
		}
	}
...
function get_nickname($uid = 0)
{
	...
	$nickname = $User->getFieldByUid('nickname',$uid);
...
function get_username($uid = 0)
{
	static $list;
	if (!($uid && is_numeric($uid))) { //获取当前登录用户名
		return $_SESSION['duodu_home']['user_auth']['username'];
	}
