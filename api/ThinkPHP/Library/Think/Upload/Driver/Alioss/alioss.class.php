<?php
namespace Think\Upload\Driver\AliOSS;
use Think\Upload\Driver\Alioss\Alioss_MimeTypes;
use Think\Upload\Driver\Alioss\Alioss_\RequestCore;
use Think\Upload\Driver\Alioss\Alioss_\ResponseCore;
...
//加载\RequestCore
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'requestcore.class.php';

//加载MimeTypes
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'mimetypes.class.php';




//检测语言包
if(file_exists(OSS_API_PATH.DIRECTORY_SEPARATOR.'zh.inc.php')){
	require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'zh.inc.php';
}else{
...
class AliOSS{
	...
	public function auth($options){
		...
		$headers = array (
			self::OSS_CONTENT_MD5 => '',self::OSS_CACHE_CONTROL => isset($options[self::OSS_CACHE_CONTROL])?$options[self::OSS_CACHE_CONTROL]:'max-age:0',
	 ...
	 * @return \ResponseCore
	 */
	public function upload_file_by_file($bucket,$object,$file,$extension,$options = NULL){
		//options
		$this->validate_options($options);
...
// 		$extension = array_pop ( $extension );
		...
		$options[self::OSS_CACHE_CONTROL]='max-age=60480000';
