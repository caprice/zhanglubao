...
class CloudsearchDoc  {
  ...
  private function action($docs, $tableName, $signMode = self::SIGN_MODE) {
    ...
    if (!is_array($docs) || empty($docs) || !is_array($docs[0])) {
      throw new \Think\Exception('Operation failed. The docs is not correct.');
    }
  ...
  public function pushCSVFile($fileName, $primaryKey, $tableName,
    ...
    while ($data = fgetcsv($reader, 1024, self::CSV_SEPARATOR)) {
      if ($lineNo == 0) {
        ...
        if (count(array_flip($data)) != count($data)) {
          throw new \Think\Exception('There are some multi fields in your header.');
        }

        $primaryKeyPos = array_search($primaryKey, $header);
        if (false === $primaryKey) {
          throw new \Think\Exception("The primary key '{$primaryKey}' is not exists.");
        }
      } else {
        if ($lineNo < $offset) {
          continue;
        ...
        if (count($data) != count($header)) {
          throw new \Think\Exception("The number of columns of values is not matched
        ...
        if ($currentSize + $totalSize >= self::PUSH_MAX_SIZE * 1024 * 1024) {
          ...
          if ('OK' != $return['status']) {
            throw new \Think\Exception("Api returns error: " . $txt .
    ...
    if (!empty($buffer)) {
      ...
      if (self::PUSH_RETURN_STATUS_OK != $return['status']) {
        throw new \Think\Exception($return['errors'][0]['message'] .
  ...
  public function pushHADocFile($fileName, $tableName, $offset = 1,
    ...
    try {
      while($line = fgets($reader)) {
        ...
        if ($separator == self::HA_DOC_ITEM_SEPARATOR) {
          ...
          if ($currentSize + $totalSize >= self::PUSH_MAX_SIZE * 1024 * 1024) {
            ...
            // 如果push不成功则抛出异常。
            if ('OK' != $return['status']) {
              throw new \Think\Exception("Api returns error. " . $return['errors'][0]['message']);
            } else {
      ...
      if (!empty($doc['fields'])) {
        throw new \Think\Exception('Fail to push doc:' . json_encode($doc));
      }
  ...
  private function _connect($fileName) {
    ...
    if (!$reader) {
      throw new \Think\Exception("The file is not exists or not readabled. Please
          check your file.");
    }
    return $reader;
  }
  ...
  private function _parseHADocField($string) {
    ...
    if ($pos !== false) {
      ...
      throw new \Think\Exception('The are no key and value in the field.');
    }
  }
